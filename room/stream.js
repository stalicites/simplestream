...
startStreamButton.addEventListener('click', async function () {
  let camOutput = await navigator.mediaDevices.getUserMedia({ video: true, audio: false });
  output.srcObject = camOutput;
...
function createChat(message, username, color) {
  ...
  chat.innerHTML = `<b style="color:${color}">${username}</b> – ${message}`.replace(/</g, "&lt;")
